...
// === Helper: init module to create VaultRegistry shared object ===

/// Triggers vault_core::init (automatic on publish) so VaultRegistry is shared.
/// Must call ts::next_tx after this to pick up the shared object.
fun init_module(scenario: &mut ts::Scenario) {
    vault_core::init_for_testing(ts::ctx(scenario));
}

// =========================================================================
// Flow 1: Init vault + grant roles
// =========================================================================

#[test]
/// Leader inits vault, receives OfficerCap + Heartbeat. Vault registered in registry.
fun init_vault_creates_shared_vault_and_caps() {
    ...
    vault_roles::init_guild_vault(&mut registry, GUILD_ID, 60_000u64, &c, ts::ctx(&mut scenario));
    ...
    let registry = ts::take_shared<VaultRegistry>(&scenario);
    ...
    // Registry should have 1 vault
    assert!(vault_registry::vault_count(&registry) == 1);
...
#[test]
/// Leader grants member cap, member receives it.
...
#[test]
/// Leader grants officer cap to another user.
...
// =========================================================================
// Flow 2: Create + claim ARCHIVE capsule
// =========================================================================
...
fun archive_capsule_full_flow() {
    ...
    vault_capsule_api::create_capsule(
        &mut vault, &member_cap,
        vault_core::mode_archive(), 10_000, @0x0,
        b"blob_archive", b"seal_archive",
...
fun archive_claim_too_early_fails() {
    ...
    vault_capsule_api::create_capsule(
        &mut vault, &member_cap,
        vault_core::mode_archive(), 10_000, @0x0,
        b"blob", b"seal", &c, ts::ctx(&mut scenario),
    );
...
fun private_inherit_full_flow() {
    ...
    vault_capsule_api::claim_private_inherit(&mut vault, 0, &c, ts::ctx(&mut scenario));
...
fun dead_man_switch_full_flow() {
    ...
    let mut registry = ts::take_shared<VaultRegistry>(&scenario);
    vault_roles::init_guild_vault(&mut registry, GUILD_ID, 5_000u64, &c, ts::ctx(&mut scenario));
    ts::return_shared(registry);
    ...
    vault_heartbeat_api::trigger_dead_man(
        &mut vault, &heart, &officer_cap, 0, &c, ts::ctx(&mut scenario),
    );
...
fun dead_man_not_timed_out_fails() {
    ...
    let mut registry = ts::take_shared<VaultRegistry>(&scenario);
    vault_roles::init_guild_vault(&mut registry, GUILD_ID, 10_000u64, &c, ts::ctx(&mut scenario));
    ts::return_shared(registry);
...
fun view_functions_return_correct_data() {
    ...
    vault_capsule_api::create_capsule_as_officer(
        &mut vault, &officer_cap,
        vault_core::mode_private_inherit(), 50_000, BENEFICIARY,
        b"view_blob", b"view_seal",
        &c, ts::ctx(&mut scenario),
    );
...
// =========================================================================
// Flow 8: VaultRegistry — multiple vaults tracked
// =========================================================================

#[test]
/// Multiple vaults are tracked in the registry with correct metadata.
fun registry_tracks_multiple_vaults() {
    let mut scenario = ts::begin(LEADER);
    let c = clock::create_for_testing(ts::ctx(&mut scenario));
    init_module(&mut scenario);
    ts::next_tx(&mut scenario, LEADER);
    ...
    // Create vault 2 with different guild
    let guild_2: address = @0x200;
    vault_roles::init_guild_vault(&mut registry, guild_2, 30_000u64, &c, ts::ctx(&mut scenario));

    assert!(vault_registry::vault_count(&registry) == 2);

    // Verify entries
    let addr_0 = vault_registry::vault_at(&registry, 0);
    let addr_1 = vault_registry::vault_at(&registry, 1);
    assert!(vault_registry::vault_guild_id(&registry, addr_0) == GUILD_ID);
    assert!(vault_registry::vault_guild_id(&registry, addr_1) == guild_2);
    assert!(vault_registry::vault_creator(&registry, addr_0) == LEADER);
    assert!(vault_registry::vault_creator(&registry, addr_1) == LEADER);
    assert!(vault_registry::has_vault(&registry, addr_0));
    assert!(vault_registry::has_vault(&registry, addr_1));

    // list_vaults pagination
    let page = vault_registry::list_vaults(&registry, 0, 10);
    assert!(page.length() == 2);

    let page_1 = vault_registry::list_vaults(&registry, 1, 1);
    assert!(page_1.length() == 1);
