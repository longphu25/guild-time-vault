...
**One‑liner**  
LoreLock là một Smart Assembly gắn với kho của hội (guild storage), cho phép guild tạo các "time‑locked capsules" chứa tài sản và bí mật, chỉ những thành viên được cấp quyền mới có thể xem và sử dụng, với hỗ trợ treasure hunts, inheritance và dead man’s switch.
...
Smart Assemblies hiện tại (Smart Storage Unit, Smart Turret, Smart Gate…) cho phép cắm thêm lớp smart contract để trở thành marketplace, quest giver, bounty board, v.v.; LoreLock là một lớp hạ tầng như vậy, tập trung vào quản lý tài sản và bí mật của hội.[web:56][web:135]
...
1. Cung cấp một **LoreLock Guild Vault Smart Assembly** có phân quyền member/officer rõ ràng trên Sui bằng capability pattern (không hard‑code danh sách địa chỉ).[web:113][web:121]  
...
1. **Guild Secret Vault**  
   - Leader init một LoreLock Guild Vault gắn với Smart Storage/structure.  
...
2. **Time‑Locked Guild Archive**  
   - Officer tạo capsule chứa câu chuyện, tài liệu, log chiến dịch, đặt unlock sau X ngày.  
...
6. **World / Smart Assembly Integration (v1)**  
   - LoreLock được triển khai như custom contract tương thích với world‑contracts, sử dụng witness type để được phép tương tác với World khi cần (ví dụ trong phiên bản nâng cao: chuyển item từ storage).[web:48][web:52][web:6]  
...
2. **Seal Integration**  
   - Sử dụng Seal SDK để:
     - Encrypt nội dung (message, file, JSON) theo policy app‑specific liên kết với module LoreLock. [web:102][web:95][web:109]  
...
3. **dApp / UI (Web)**  
   ...
   - Màn hình tối thiểu:
     - **LoreLock Vault Dashboard**: list capsules theo mode, trạng thái (locked/unlocked/claimed).  
...
- **Khả năng nâng cấp**:  
  - Module LoreLock thiết kế với cấu trúc dữ liệu versionable (có thể thêm mode, thêm field) mà không phá vỡ dữ liệu cũ. [web:117][web:79]  
